...
{
    Vector2 circleWorldPos = GetWorldPos();
    float circleRadius = GetWorldRadius();

    float left = GetWorldPos().x - circleRadius;
    float right = GetWorldPos().x + circleRadius;
    float top = GetWorldPos().y + circleRadius;
    float bottom = GetWorldPos().y - circleRadius;

    if (right >= col->GetAABB_Info().left && left <= col->GetAABB_Info().right &&
        top >= col->GetAABB_Info().bottom && bottom <= col->GetAABB_Info().top)
    {
        return true;
    }
